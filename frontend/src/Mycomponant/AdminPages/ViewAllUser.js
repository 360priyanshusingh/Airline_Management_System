...
const handleFilter = (e) => {
    ...
    const newdata = data.filter(row => {
        ...
        const countryMatch = row.country.toLowerCase().includes(searchTerm);
        const maleMatch = row.gender.toLowerCase().includes(searchTerm);
        const cityMatch = row.city.toLowerCase().includes(searchTerm);
        return firstNameMatch || lastNameMatch || emailMatch || countryMatch || maleMatch || cityMatch ;
    });
    setRecord(newdata);
 } 
